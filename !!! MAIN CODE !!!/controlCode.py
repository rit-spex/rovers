    ...
    while True:
        ...
        for i in [-1, 0, 1, 0]:  # Run loop for the values [-1,0,1,0]
            ...
            while currMotor > 0:
                ...
                print(
                    "Python: Set the duty percentage to {}.".format(dutyLeft)
                )  # Tell user what the pi is telling arduino
...
def receiveXboxSignals(cont):
    ...
    for event in pygame.event.get():
        ...
        ):  # Code for button press  # NOTE: MAPPING NEEDS VERIFICATION
...
def calcDutyCycle(signal):
    ...
    if (signal >= -0.01) and (signal <= 0.09):  # If joysticks are likely untouched
        ...
        duty = (
            -10 + 3 * slowMode
        ) * signal + offset  # Run Conversion algorithm for joystick to dutyCycle
    return duty
...
def buttonPressEvent(event):
    ...
    if event.button == 11:  # Middle Right "Menu" Button
        ...
        if len(aPresses) >= 3:  # If there have been least 3 clicks
            ...
            if (
                currentTime - aPresses[-2] < 1
            ):  # If the last three clicks all happened within one second
...
def setDuty(motorCode, dutyCycle):
    ...
    stepValue = 100
    if (
        abs((tempNew) - (curr)) >= stepValue
    ):  # If the distance between the current and desired is > 1
