...
def connect():
    ...
    while True:  # Loop until the connection has been established
        try:
            ...
            )  # Enable arduino communication (pi)
            ...
            print(
                "ERROR : The arudino is not connected to the Pi. Attempting to reconnect..."
            )  # Inform that the connection has not been made
...
def tellArm(gCode):
    ...
    try:
        ...
        print(
            "Arduino is no longer connected !!!"
        )  # Warn about a disconnection if transmittion fails
